...
from utils.credentials import refresh_credentials as refresh_creds_util
from utils import get_message_limit, load_credentials_from_db, reset_message_limit, save_credentials_to_db, getCurrentTime, transformPhoneNumber
...
class User:
	...
	def refresh(self):
		...
		self.message_limit_current = message_limit[3]
	...
	# * Delegates to utils/credentials.py — removed duplicate implementation
	def save_credentials_to_db(self, user_id: int, credentials):
		save_credentials_to_db(user_id, credentials)

	# * Delegates to utils/db.py — removed duplicate implementation
	def load_credentials_from_db(self, user_id: int):
		return load_credentials_from_db(user_id)

	# * Delegates to utils/credentials.py — removed duplicate implementation
	def refresh_credentials(self, user_id: int):
		try:
			return refresh_creds_util(user_id)
	...
	def get_message_limit(self):
		return get_message_limit(self.user_id)

	def reset_message_limit(self, date_now: str):
		reset_message_limit(self.user_id, date_now)
	...
	def update_account_details(self, email: str, wNumber: str, wToken: str, gSheetID: str, price_lead: int, template_name: str, name_col: str, phone_col: str) -> bool:
		...
		with sqlite3.connect("database.db") as conn:
			cursor = conn.cursor()
			cursor.execute("UPDATE users SET email = ?, whatsapp_key = ?, whatsapp_id = ?, sheet_id = ?, price_lead = ?, template_name = ?, column_name = ?, column_phone = ? WHERE user_id = ?",
				(email, wToken, wNumber, gSheetID, price_lead, template_name, name_col, phone_col, self.user_id))
			conn.commit()
		return True
	...
	# BUG: calls reset_message_limit(self, date_now) as standalone — kept as-is
	def messageLimit(self):
		result = self.get_message_limit()
		date_now = datetime.now().strftime("%Y-%m-%d")

		if result[1] == 0:
			return True
		elif result[4] != date_now:
			reset_message_limit(date_now)
			return True
	...
	# BUG: calls messageLimit() without self — kept as-is
	# BUG: references undefined variable `count` — kept as-is
	def sender(self):
		if self.messageLimit() == False:
			print(f"[{getCurrentTime()}] Message limit reached: {self.message_limit_value} messages sent today.")
			self.stop_listener()
			return None
